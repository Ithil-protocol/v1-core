...
let createStrategy: ThenArg<ReturnType<typeof aaveFixture>>["createStrategy"];
...
describe("Aave strategy integration tests", function () {
  ...
  before("prepare vault with default parameters", async () => {
    ...
    investmentTokenDAI = <ERC20>await ethers.getContractAt(tokenArtifact.abi, aDAI);
  ...
  it("Aave Strategy: open position on DAI", async function () {
  ...
  it("Aave Strategy: close position on DAI", async function () {
    const initialVaultBalance = await marginTokenDAI.balanceOf(vault.address);
