...
export function checkEditPosition(): void {
  it("MarginTradingStrategy: editPosition", async function () {
    ...
    const newCollateral = marginTokenMargin.div(2);
    await marginToken.connect(trader).approve(this.marginTradingStrategy.address, newCollateral);
    await this.marginTradingStrategy.connect(trader).editPosition(1, newCollateral);
