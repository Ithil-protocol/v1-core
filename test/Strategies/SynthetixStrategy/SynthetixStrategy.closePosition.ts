...
export function checkClosePosition(): void {
  it("SynthetixStrategy: closePosition", async function () {
    ...
    await marginToken.connect(trader).approve(this.synthetixStrategy.address, marginTokenMargin);
    ...
    await this.synthetixStrategy.connect(trader).openPosition(order);

    await changeRate(this.mockKyberNetworkProxy, investmentToken, 11 * 10 ** 10);
    await this.synthetixStrategy.connect(trader).closePosition(1);
